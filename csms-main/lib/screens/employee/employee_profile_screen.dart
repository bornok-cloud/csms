            ...
            child: Padding(
              ...
              child: Column(
                children: [
                  ...
                  Text(user.position ?? 'Employee', style: const TextStyle(color: AppColors.accent, fontWeight: FontWeight.w600)),
                  const Divider(height: 32),
                  _row(Icons.badge_outlined, 'Employee ID', user.employeeId ?? '—'),
                  _row(Icons.work_outline, 'Position', user.position ?? '—'),
                  _row(Icons.email_outlined, 'Email', user.email),
                  _row(Icons.phone_outlined, 'Phone', user.phone),
                  const SizedBox(height: 20),
                  Row(children: [
                    Expanded(child: OutlinedButton(onPressed: () => showAppSnack(context, 'Profile updated (mock).'), child: const Text('Edit Info'))),
